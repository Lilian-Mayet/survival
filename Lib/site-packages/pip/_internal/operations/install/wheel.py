...
def message_about_scripts_not_on_PATH(scripts: Sequence[str]) -> Optional[str]:
    ...
    for parent_dir, dir_scripts in warn_for.items():
        ...
        if len(sorted_scripts) == 1:
            start_text = "programme {} is".format(sorted_scripts[0])
        else:
    ...
    last_line_fmt = (
        ...
        "to suppress this warning, use --no-warn-programme-location."
    )
    if len(msg_lines) == 1:
...
class MissingCallableSuffix(InstallationError):
    def __init__(self, entry_point: str) -> None:
        super().__init__(
            "Invalid programme entry point: {} - A callable "
...
def _install_wheel(
    ...
    #   generated = files newly generated during the install (programme wrappers)
    installed: Dict[RecordPath, RecordPath] = {}
    ...
    def is_entrypoint_wrapper(file: "File") -> bool:
        # EP, EP.exe and EP-programme.py are scripts generated for
        # entry point EP by setuptools
        path = file.dest_path
        name = os.path.basename(path)
        if name.lower().endswith(".exe"):
            matchname = name[:-4]
        elif name.lower().endswith("-programme.py"):
            matchname = name[:-10]
