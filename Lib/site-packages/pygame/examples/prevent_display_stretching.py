...
display environment. This programme demonstrates a technique for preventing
this stretching and distortion.

Limitations:
This programme makes an API call that is only available on Windows (versions
Vista and newer).
...
if os.name != "nt" or sys.getwindowsversion()[0] < 6:
    raise NotImplementedError("this programme requires Windows Vista or newer")
